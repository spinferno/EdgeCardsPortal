...
function setErrorMessage(fieldName, message) {
    var html = '<div id="' + fieldName +
        '_errormessage" class="validation-summary alert alert-error alert-danger alert-block inlineValidation" role="error" tabindex="-1">' +
        message + '</div>';
    $("#" + fieldName).closest(".control").after(html);
}
...
function showHideDiabetiesMoreDetails() {
    if ($("#vr_wellmanageddiabetesyesnoblank").val() == YESNODONTKNOW.NO || $("#vr_hypoglyceamicawareness").val() ==
        YESNODONTKNOW.NO) {
...
function showHideVisionOrderBased(isVisible, clearValue) {
    ...
    if (!isChecked && !checkedNo) { // not checked
...
function recomendationOnload() {
    var fieldGroupHide = ["vr_onroaddrivingassessmentprivate", "vr_savetodriveprivateyesnoblank",
        ...
        "vr_correctivelensestobewornwhiledriving", "vr_dateofreport", 
        "vr_onroaddrivingassessment", "vr_specialistreview"
    ];
    setFieldGroupVisible(fieldGroupHide, false);
}
//Details
...
function addRangeValidatorForCardio(fieldName) {
    ...
    var evaluationfunction = function () {
        ...
        if (((window.$.isNumeric(fieldValue) && fieldValue >= Validation.MinRange &&
            fieldValue <= Validation.MaxRange) || fieldValue === "")) {
            return true;
        } else {
            return false;
        }
...
function addRangeValidatorForAided(fieldName) {
    ...
    var evaluationfunction = function () {
        ...
        if (window.$("#vr_requirecorrectivelenseswhiledriving_1").is(":checked") && !(window.$.isNumeric(fieldValue) &&
            fieldValue >= Validation.MinRange && fieldValue <= Validation.MaxRange)) {
            return false;
        } else {
            return true;
        }
...
function specifySeziureDateValidation() {
    ...
    var evaluationfunction = function (e) {
        ...
        if ($("#vr_hadseizure_1").is(":checked") && !$("#vr_idontknowtheexactseizuredate").is(":checked") &&
            fieldValue == "") {
...
function MatchCase() {
    ...
    matchCaseNumberAndLicenceNumber(caseNumber, licenceNumber,
        (function (result) {
...
function getAttachmentFileNames(medicalreportnumberId, callback) {
    if (medicalreportnumberId == null || medicalreportnumberId == "")
        return;
...
MedicalConditionText = {
    ...
    SeizureEpilepsy: "Seizure and epilepsy",
...
function setMedicalConditions() {
    ...
    if (!window.$("#vr_idonthavethisinformation").is(":checked")) {
        ...
        getMedicalCondition(customerId,
            (function (result) {
                if (result && result.length > 0) {
                    ...
                    getMedicalEvent(customerId,
                        (function (resultEvents) {
                            if (resultEvents && resultEvents.length > 0) { customerExistingMedicalEvents = resultEvents; }
                        }));

                    // Check for Vision and eye disorder conditions 
                    if (checkIfMedicalConditionExists(visionConditions, customerExistingMedicalConditions))
                        preExistingConditions += MedicalConditionText.VisionEyeDisorders + ", ";

                    if (isMedicalReport()) { // Below pre-exisitng conditions to be shown only for Medical Report, not Eyesight Report

                        // Check for blackout/s conditions and any blackout medical event
                        if (checkIfMedicalConditionExists(blackoutConditions, customerExistingMedicalConditions) || checkIfMedicalEventExists(blackoutMedicalEvents, customerExistingMedicalEvents))
                            preExistingConditions += MedicalConditionText.Blackouts + ", ";

                        // Check for cardiovascular conditions and any cardiovascular medical event
                        if (checkIfMedicalConditionExists(cardiovascularConditions, customerExistingMedicalConditions) || checkIfMedicalEventExists(cardiovascularMedicalEvent, customerExistingMedicalEvents))
                            preExistingConditions += MedicalConditionText.Cardiovascular + ", ";

                        // Check for diabetes condition 
                        if (checkIfMedicalConditionExists(diabetesConditions, customerExistingMedicalConditions))
                            preExistingConditions += MedicalConditionText.Diabetes + ", ";

                        // Check for musculoskelatal condition 
                        if (checkIfMedicalConditionExists(musculoskeletalDisordersConditions, customerExistingMedicalConditions))
                            preExistingConditions += MedicalConditionText.MusculoskeletalDisorders + ", ";

                        // Check for dementia conditions
                        if (checkIfMedicalConditionExists(dementiaConditions, customerExistingMedicalConditions))
                            preExistingConditions += MedicalConditionText.Dementia + ", ";

                        // Check for seizure and epilepsy conditions and any seizure medical event
                        if (checkIfMedicalConditionExists(seizureAndEpilepsyConditions, customerExistingMedicalConditions) || checkIfMedicalEventExists(seizureAndEpilepsyMedicalEvents, customerExistingMedicalEvents))
                            preExistingConditions += MedicalConditionText.SeizureEpilepsy + ", ";

                        // Check for neurological conditions and any neurological medical event
                        if (checkIfMedicalConditionExists(neurologicalConditions, customerExistingMedicalConditions) || checkIfMedicalEventExists(neurologicalMedicalEvents, customerExistingMedicalEvents))
                            preExistingConditions += MedicalConditionText.Neurological + ", ";

                        // Check for mental health conditions 
                        if (checkIfMedicalConditionExists(mentalHealthConditions, customerExistingMedicalConditions))
                            preExistingConditions += MedicalConditionText.MentalHealth + ", ";

                        // Check for sleep disorder conditions 
                        if (checkIfMedicalConditionExists(sleepDisordersConditions, customerExistingMedicalConditions))
                            preExistingConditions += MedicalConditionText.SleepDisorders + ", ";

                        // Check for substance misuse conditions 
                        if (checkIfMedicalConditionExists(substanceMisuseConditions, customerExistingMedicalConditions))
                            preExistingConditions += MedicalConditionText.SubstanceMisuse + ", ";

                        // Check for hearing loss conditions 
                        if (checkIfMedicalConditionExists(hearingLossConditions, customerExistingMedicalConditions))
                            preExistingConditions += MedicalConditionText.HearingLoss + ", ";
                    }
                    if (preExistingConditions != "")
                        preExistingConditions = MedicalConditionText.PrefixText + preExistingConditions.slice(0, -2);
                }
            }));
...
function displayPreExistingMedicalConditions() {
    if (sessionStorage.getItem("preExistingConditions") != null)
        window.$("#preExistingConditions").text(sessionStorage.getItem("preExistingConditions"));
    else
        window.$("#preExistingConditions").text("");
}
...
function checkIfMedicalConditionExists(conditions, customerExistingMedicalConditions) {
    if (conditions != null && customerExistingMedicalConditions != null) {  // Null check added
        for (var i = 0; i < conditions.length; i++) {
            if (customerExistingMedicalConditions[conditions[i]] == "true")
                return true;
        }
    }
    return false;
...
function checkIfMedicalEventExists(medicalEventConditions, customerExistingMedicalEvents) {
    if (medicalEventConditions != null && customerExistingMedicalEvents != null) {  // Null check added
        for (var i = 0; i < medicalEventConditions.length; i++) {
            for (var j = 0; j < customerExistingMedicalEvents.length; j++) {
                if (customerExistingMedicalEvents[j]["vr_medicaleventtype"] == medicalEventConditions[i])
                    return true;
            }
        }
    }
...
// Clear session variables after report is submitted/completed
function clearReportRelatedSessionVariables() {
    sessionStorage.removeItem("customerid");
    sessionStorage.removeItem("preExistingConditions");
...
function addMaxCharLimitValidation(fieldName, charlimit) {
    var evaluationfunction = function () {
        ...
        if (nCRLF != null) { nCRLF = nCRLF.length * 2; } else { nCRLF = 0; } // Carriage returns are considered as 2 characters by browser
        var totalCount = nChar + nCRLF;
        if (totalCount > charlimit) { return false; } else { return true; }
    };
...
// CMPP-6159 - ADD ACTION BUTTON ON WEB FORM
function addActionButton(buttonCode) {
    var defaultButtonCode = "<input type=\"button\" name=\"SaveDraftButton\" value=\"Save as Draft\" onclick=\"onSaveAsDraft();\" id=\"SaveDraftButton\" class=\"btn btn-default button save-draft save-draft-btn\"></input>";
...
// CMPP-6159 - ON CLICK OF SAVE AS DRAFT, SET "saveasdraft" SESSION VARIABLE
function onSaveAsDraft() {
    var currentPage = $("#EntityFormView > h2").text();
    if (currentPage.startsWith("Patient")) {
        goAttachment(true);
    }
    else if (currentPage.startsWith("Attachment")) {
        sessionStorage.setItem("saveasdraft", true);
        $("#SaveDraftButton").val("Saving...");
        $("#NextButton").click();
        $("#NextButton").val("Recommendation");
    }
    else if (currentPage.startsWith("Recommendation")) {
        if (window.Page_ClientValidate('')) {
            sessionStorage.setItem("saveasdraft", true);
            $("#SaveDraftButton").val("Saving...");
            $("#NextButton").click();
            $("#NextButton").val("Review");
        }
    }
}

// CMPP-6159 - IF SAVE AS DRAFT IS CLICKED, GO BACK TO PREVIOUS STEP
function isSaveAsDraftClicked() {
    return (sessionStorage.getItem("saveasdraft") == "true")
}

// CMPP-6159 - IF SAVE AS DRAFT IS CLICKED, GO BACK TO PREVIOUS STEP
function actionSaveAsDraft() {
